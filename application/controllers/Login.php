...
class Login extends BaseController
{
    ...
    public function authgoogle(){
      ...
      if($result->num_rows()>0){
        ...
        $this->session->set_flashdata('error', 'akun google anda tidak terdaftar, <a href="https://wa.me/628993932789" target="_blank">klik ini<a/> untuk meminta admin menghubungkan akun dengan google anda!');
        redirect('login');
      }
    }

    public function authgoogleAdmin(){
      $profile    = $this->userlogin->oauth2_google_admin()['profile'];
      $email      = $profile['email'];
      $result     = $this->auth_model->authgoogle($email);
