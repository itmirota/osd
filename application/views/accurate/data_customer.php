          ...
          <tr>
            ...
            <td>
              <div class="d-grid gap-2 d-md-block">
                ...
                <button class="btn btn-sm btn-primary process" data-id="<?=$ld->id_customer?>" type="button"><i class="fa fa-solid fa-check"></i></button>
                <?php } ?>
  ...
  document.querySelectorAll('.process').forEach(button => {
    button.addEventListener('click', async function () {
      ...
      button.disabled = true;
          ...
          try {
      const response = await fetch('updateProcessCustomer', {
        method: 'POST',
        headers: {
      ...
      if (result.status === 'success') {
          Swal.fire({
            ...
            window.location.href = "<?= base_url('data-customer'); ?>";
          });
      } else {
