			...
			<div class="card-body">
				...
				<form id="register">
					...
					<div class="d-grid gap-2 col-10 mx-auto">
						<button type="submit" class="btn btn-success"> <b><?= $jenis_absen ?></b></button>
					</div>
				</form>
			</div>
		</div>
	...
	<script type="text/javascript">
		$('#register').on('submit', function (event) {
			...
			function save(data_uri,lat,lon){
				...
				$.ajax({
					url: '<?php echo site_url("absensi/simpanIstriahat");?>',
					type: 'POST',
					dataType: 'json',
				...
				.done(function(data) {
					if (data > 0) {
						window.location.href="<?php echo base_url(); ?>istirahat";
